...
APIS_CMODE = ['ApiProxy', 'DatasetListInfoIterStart', 'DatasetListInfoIterNext',
    'DatasetListInfoIterEnd', 'DatasetEditBegin', 'DatasetEditCommit',
    'DatasetProvisionMember', 'DatasetRemoveMember', 'DfmAbout',
    'DpJobProgressEventListIterStart', 'DpJobProgressEventListIterNext',
    'DpJobProgressEventListIterEnd', 'DatasetMemberListInfoIterStart',
...
CMODE_APIS =['provisionLun','destroyLun','cloneLun','mapLun','unmap','listLuns',
             'getLunTargetDetails']
...
class FakeCMODEServerHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_GET(s):
        """Respond to a GET request."""
        if '/axis2/services/CloudStorageService?wsdl' != s.path:
            s.send_response(404)
            s.end_headers
            return
        s.send_response(200)
        ...
        for api in CMODE_APIS:
            ...
            out.write('</operation>')
        out.write('</portType>')
        out.write('<binding name="CloudStorageBinding" type="na:CloudStorage">')
        out.write('<soap:binding style="document" ' +
            'transport="http://schemas.xmlsoap.org/soap/http"/>')
        for api in CMODE_APIS:
            out.write('<operation name="%s">' % api)
    ...
    def do_POST(s):
        ...
        body = root.xpath('/soapenv:Envelope/soapenv:Body', namespaces=nsmap)[0]
        request = body.getchildren()[0]
        tag = request.tag
        if not tag.startswith('{' + ntap_ns + '}'):
            s.send_response(500)
...
class NetAppCmodeISCSIDriverTestCase(test.TestCase):
    """Test case for NetAppISCSIDriver"""
    volume = {'name':'lun1','size':1,'volume_name':'lun1','os_type':'linux',
    ...
    def test_map_unmap(self):
        self.driver.create_volume(self.volume)
        updates = self.driver._get_export(self.volume)
        self.assertTrue(updates['provider_location'])
